...
18. Disconnect the Android device from the development host and connect it to the station.
19. Turn on the station.
20. Ensure **Upate Firmware** is enabled.
...
## TC-023 — Firmware File Constraints
...
Verify that firmware files smaller than 48 bytes, larger than 200 kB, or
having an extension other than `.bin` are not presented as selectable
repository firmware, and that attempting to select such files through
**Browse** produces the appropriate validation error.
...
* `too_small.bin` — exactly 47 bytes and has a `.bin` extension.

### Preconditions

1. The firmware repository is configured to the test repository containing the test files listed above.
2. The Android device has an Internet connection.
...
1. Open the application.
2. Tap **Select Firmware**.
3. Verify that `valid_exactly_200kB.bin` is listed.  Note that if other valid files are present in the repository, they will also be listed.
...
7. Verify that `too_small.bin` is not listed.
8. Tap **Browse**.
9. Navigate to the test files and select `too_small.bin`.
10. Verify that the application rejects the file and displays the error message indicating that the file is too small.
11. Tap **Browse** again.
12. Navigate to the test files and select `too_large.bin`.
13. Verify that the application rejects the file and displays the error message indicating that the file is too large.
14. Tap **Browse** again.
15. Navigate to the test files and select `wrong_extension.txt`.
16. Verify that the application rejects the file and displays the error message indicating that the file must have a `.bin` extension.
17. Tap **Browse** again.
...
* As described in the procedure and invalid Browse selection does not become the selected firmware.
